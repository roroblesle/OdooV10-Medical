...
class OeHealthSurgeryTeam(models.Model):
    ...
    @api.multi
    def onchange_team_member(self,team_member):
        res = {}
        physician_obj = self.env['oeh.medical.physician']
        if team_member:
            phy = physician_obj.browse(team_member)
            if phy.speciality:
                role = phy.speciality.id
                res = {'value': {'role': role}}
        return res
...
class OeHealthSurgery(models.Model):
    ...
    procedures = fields.Many2many('oeh.medical.procedure', 'oeh_surgery_procedure_rel', 'surgery_id', 'procedure_id', string='Procedures', help="List of the procedures in the surgery. Please enter the first one as the main procedure", readonly=True,states={'Draft': [('readonly', False)]})
    ...
    surgery_length = fields.Float(compute=_surgery_duration, string='Duration', help="Length of the surgery", readonly=True,states={'Draft': [('readonly', False)]})
    ...
    info = fields.Text(string='Extra Info', readonly=True, states={'Draft': [('readonly', False)]})
    anesthesia_report = fields.Text(string='Anesthesia Report', readonly=True, states={'Draft': [('readonly', False)]})
    ...
    supplies = fields.One2many('oeh.medical.surgery.supply', 'name', string='Supplies', help="List of the supplies required for the surgery", readonly=True, states={'Draft': [('readonly', False)]})
    ...
    def action_surgery_sign(self):
        ...
        if user_ids:
            user = user_ids.id or False
        else:
            raise UserError(_('No physician associated to logged in user'))
        return self.write({'state': 'Signed', 'signed_by': user})
